...
def binary_order_1():
    ...
    def map2inputs(repo):
        return int(repo[0] * M + 0.5) * (M + 1) + int(repo[1] * M + 0.5)
    learner = OnlineLearning(experts, map2inputs, (M + 1) * (M + 1))
    learner.train(N=10000)
    minloss, minweight, func = learner.loss, learner.weight, learner.func
    avefunc = get_average(learner.allfuncs)
    aveweight = get_average(learner.allweights)
    
    learner.test(aveweight, avefunc)
...
def save_binary_function(M=50, N=100):
    def map2inputs(repo):
        return int(repo[0] * M + 0.5) * (M + 1) + int(repo[1] * M + 0.5)
    ...
    for i in range(M + 1):
        for j in range(M + 1):
            ...
            if (t * t - 4 * (i / M)) < 0:
                continue
            s = math.sqrt(t * t - 4 * (i / M))
            u = (t + s) / 2
            v = t - u
            u = int(u * 100000 + 0.5) / 100000
            v = int(v * 100000 + 0.5) / 100000
            realfunc[i][j] = func[map2inputs((u, v))]
            lower[i][j] = int(lowerfunc[map2inputs((u, v))] * 100 + 0.5)
            upper[i][j] = int(upperfunc[map2inputs((u, v))] * 100 + 0.5)
            #myfunc[i][j] = int(func[map2inputs((u, v))] * 100 + 0.5)
            myfunc[i][j] = int(BinaryAggregator((u, v)) * 100 + 0.5)
            if (i > 0) and (j > 0) and (i < M) and (j < M):
                pnasfunc[i][j] = int(((u) * (v)) / ((u) * (v) + (i + j) / M / 2 * (1 - u) * (1 - v) / (1 - (i + j) / M / 2)) * 100 + 0.5)
...
def binary_order_2_iid(noise=0.0):
    ...
    while True:
        ...
        if (cur[0] < cur[1]) and (cur[1] < cur[2]) and (cur[3] < cur[4]):
            model = BinaryOrder2IID(reports=[[cur[0] / M, cur[1] / M, cur[2] / M] for i in range(2)], 
                E_0=[cur[3] / (N / M) for i in range(2)], E_1=[cur[4] / (N / M) for i in range(2)])
            if model.args != None:
    ...
    learner.train(N=10000, info_epoch=100)
    # minloss, minweight, func = learner.loss, learner.weight, learner.func
    # avefunc = get_average(learner.allfuncs)
    # aveweight = get_average(learner.allweights)
    
    learner.test(k=10, output=True)
    # learner.output_topweighted(k=40, output=True)

def report2signal():
    M = 5
    experts = []
    enu = Enumerator(end=(M * 20, M, M))
    while True:
        # cur = enu.cur[4:] + enu.cur[:4]
        cur = enu.cur
        if (cur[1] * 10 < cur[0]) and (cur[2] * 10 >= cur[0]) and (cur[0] > 0) and (cur[0] < M * 10):
            experts.append(Binary(mu=cur[0] / M / 10, reports=[[cur[1] / M, cur[2] / M], [cur[1] / M, cur[2] / M]]))
            print(experts[-1].calc_signal(), '\t', experts[-1].args)
        if not enu.step():
            break
