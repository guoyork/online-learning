...
def print_heatmap(filename, data, labels, figsize = (7, 6)):

    fig, ax = plt.subplots(figsize = figsize)
    norm = matplotlib.colors.Normalize(0, 100)
    im, cbar = heatmap(data, labels, labels, norm = norm, ax=ax, cmap="Blues", usecbar = False)
    fmt = matplotlib.ticker.FuncFormatter(lambda x, pos: "{:.0f}".format(x))
    texts = annotate_heatmap(im, valfmt = fmt, threshold = 50)
